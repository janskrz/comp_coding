defmodule Board do
  ...
  def clear_cell(b, {x, y, z, w}) do
    %{b | :board => Map.delete(b.board, {x, y, z, w})}
  end

  def get_neighbors(b, {x, y, z, w}) do
    entries =
      for i <- (x - 1)..(x + 1),
          j <- (y - 1)..(y + 1),
          k <- (z - 1)..(z + 1),
...
defmodule Solution do
  ...
  def fill_board(board, ["#" <> rest | t], x, y),
    do: Board.set_cell(board, {x, y, 0, 0}) |> fill_board([rest | t], x + 1, y)
